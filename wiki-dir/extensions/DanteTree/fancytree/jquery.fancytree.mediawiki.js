...
(function($, window, document, mw, undefined) {
  ...
  /**
   * Open the tree to the node containing the passed title, or current page if
   * none supplied
   */
  ...
  /**
   * Register the extension and set the lazy-loading up in a MediaWiki-friendly
   * way
   */
  $.ui.fancytree.registerExtension({
    ...
    // When a tree is initialised, do some modifications appropriate to
    // mediawiki trees
    treeInit : function(ctx) {
      var tree = ctx.tree, opts = ctx.options;

      // If there's "ajax" in this nodes data, then it needs to be marked as
      // lazy and the "ajax" removed
      ...
      opts.lazyLoad = function(event, data) {
        ...
        // If the ajax option is an URL, split it into main part and
        // query-string
        if (url.match(/^(https?:\/\/|\/)/)) {
          var parts = url.split('?');
      ...
      // Parse the data collected from the Ajax response and make it into child
      // nodes
      opts.postProcess = function(event, data) {
        ...
        // Otherwise see if it's as a JSON list of node data (need to extract as
        // MediaWiki adds parser info)
        else if (m = response.match(/^.*?(\[[\s\S]+\])/i))
          data.result = $.parseJSON(m[1]);

        // Otherwise see if it's as a JSON list of node data contained in an
        // object (as it is in an API response)
        else if (m = response.match(/^\{/)) {
          var json = $.parseJSON(response);
          data.result = false;
          for ( var k in json) {
            if (data.result === false && typeof json[k] == 'array')
              data.result = json[k];
          }
        }
